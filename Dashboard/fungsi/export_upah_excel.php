...
if ($komponenPendapatan) {
    while ($komponen = mysqli_fetch_assoc($komponenPendapatan)) {
...
if ($komponenPotongan) {
    while ($komponen = mysqli_fetch_assoc($komponenPotongan)) {
...
if ($namaPendapatanManual) {
    while ($item = mysqli_fetch_assoc($namaPendapatanManual)) {
        $nama = (string) $item["nama"];
        if (isset($namaKomponenPendapatan[mb_strtolower(trim($nama))])) {
            continue;
        }
...
if ($namaPotonganManual) {
    while ($item = mysqli_fetch_assoc($namaPotonganManual)) {
        $nama = (string) $item["nama"];
        if (isset($namaKomponenPotongan[mb_strtolower(trim($nama))])) {
            continue;
        }
...
foreach ($dynamicExportColumns as $key => $column) {
    $kolomExportPilihan[$key] = ["label" => $column["label"], "sql" => "k.employee_name"];
}
...
if (!isset($kolomExportUrutan[$sortExport])) {
    $sortExport = "employee_name";
}

$where = ["1=1"];
if (roleOperasional()) {
    $where[] = "k.department_id = " . (int) ($departmentPengguna ?? 0);
}
if ($cari !== "") {
...
if ($position !== "") {
    $safePosition = mysqli_real_escape_string($conn, $position);
    $where[] = "k.position = '$safePosition'";
}
if ($bulan > 0) {
    $where[] = "MONTH(COALESCE(pg.berlaku_mulai, k.date_of_hire)) = $bulan";
}
if ($tahun > 0) {
    $where[] = "YEAR(COALESCE(pg.berlaku_mulai, k.date_of_hire)) = $tahun";
}

$sortSql = $kolomExportPilihan[$sortExport]["sql"] ?? "k.employee_name";
$limitSql = $batasExport === "semua" ? "" : " LIMIT " . (int) $batasExport;

$sql = "SELECT k.id AS karyawan_id, k.emp_id, k.employee_name, k.position, k.department, pg.id AS profil_id,
               COALESCE(pg.gaji_pokok, k.salary, 0) AS gaji_pokok,
               COALESCE(pg.uang_makan, 0) AS uang_makan,
               COALESCE(lembur.total_upah, 0) AS upah_lembur,
               COALESCE(pg.berlaku_mulai, k.date_of_hire) AS berlaku_mulai
        FROM karyawan k
        LEFT JOIN profil_gaji pg ON pg.karyawan_id = k.id
        LEFT JOIN (
            SELECT o.karyawan_id, SUM(oc.jumlah_upah) AS total_upah
        ...
        ORDER BY " . $sortSql . " " . $arahExport . ", k.employee_name ASC" . $limitSql;

$result = mysqli_query($conn, $sql);
if (!$result) {
    http_response_code(500);
    exit("Data upah gagal diproses: " . mysqli_error($conn));
}
...
if ($profilIds !== []) {
    ...
    if ($hasilKomponen) {
        while ($item = mysqli_fetch_assoc($hasilKomponen)) {
            $nilaiKomponen[(int) $item["profil_gaji_id"]][(int) $item["jenis_komponen_id"]] = (float) $item["nilai"];
        }
    }
}
...
if ($karyawanIds !== []) {
    ...
    if ($hasilManual) {
        while ($item = mysqli_fetch_assoc($hasilManual)) {
            $nilaiManual[(int) $item["karyawan_id"]]["pendapatan"][mb_strtolower((string) $item["nama"])] = ($nilaiManual[(int) $item["karyawan_id"]]["pendapatan"][mb_strtolower((string) $item["nama"])] ?? 0) + (float) $item["nilai"];
        }
    }
    ...
    if ($hasilManualPotongan) {
        while ($item = mysqli_fetch_assoc($hasilManualPotongan)) {
            $nilaiManual[(int) $item["karyawan_id"]]["potongan"][mb_strtolower((string) $item["nama"])] = ($nilaiManual[(int) $item["karyawan_id"]]["potongan"][mb_strtolower((string) $item["nama"])] ?? 0) + (float) $item["nilai"];
        }
    }
}
...
foreach ($rows as &$row) {
    foreach ($dynamicExportColumns as $key => $column) {
        $nilai = 0.0;
        if (isset($column["jenis_id"])) {
            $nilai = (float) ($nilaiKomponen[(int) ($row["profil_id"] ?? 0)][(int) $column["jenis_id"]] ?? 0)
                + (float) ($nilaiManual[(int) $row["karyawan_id"]][$column["kategori"]][mb_strtolower((string) $column["label"])] ?? 0);
        } else {
            $nilai = (float) ($nilaiManual[(int) $row["karyawan_id"]][$column["kategori"] === "pendapatan_manual" ? "pendapatan" : "potongan"][mb_strtolower((string) $column["nama"])] ?? 0);
        }
        $row[$key] = $nilai;
    }
}
...
if (isset($_GET["download"])) {
    ...
    unduhSpreadsheetXlsx("data-upah-" . date("Y-m-d"), "Data Upah", $headers, $exportRows);
}
...
<section class="form-card export-options-card">
    <div class="form-card-header">
        <h2>Opsi Export Data Upah</h2>
        <p><?= count($rows); ?> data sesuai filter.</p>
    </div>
    <div class="form-body">
        <form method="GET" class="export-options-form">
            ...
            <div class="form-group">
                ...
                <select id="sort_export" name="sort_export">
                    <?php foreach ($kolomExportUrutan as $kunci => $kolom): ?>
            ...
            <div class="form-group">
                ...
                <select id="arah_export" name="arah_export">
                    <option value="ASC" <?= $arahExport === "ASC" ? "selected" : ""; ?>>Naik</option>
                    <option value="DESC" <?= $arahExport === "DESC" ? "selected" : ""; ?>>Turun</option>
                </select>
            </div>
            <?php foreach (["cari" => $cari, "department" => $department, "bulan" => $bulan, "tahun" => $tahun, "position" => $position] as $nama => $nilai): ?>
                <input type="hidden" name="<?= $nama; ?>" value="<?= htmlspecialchars((string) $nilai); ?>">
            <?php endforeach; ?>
...
<section class="data-card">
    <div class="data-card-header">
        <h2>Pratinjau Export Data Upah</h2>
    </div>
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
                <?php if ($rows === []): ?>
                    <tr>
                        <td colspan="<?= count($kolomDipilih); ?>" class="empty-table">Belum ada data upah untuk diekspor.</td>
                    </tr>
                <?php else: ?>
