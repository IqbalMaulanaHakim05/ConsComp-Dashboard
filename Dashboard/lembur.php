...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["aksi"] ?? "") === "kompensasi" && in_array(rolePengguna(), ["pic", "admin", "superadmin"], true)) {
    ...
    if (!csrfValid($_POST["csrf_token"] ?? null) || !in_array($metode, ["per_jam", "nominal_final"], true) || $jumlah < 0 || $tarif < 0) {
        ...
        $cekSql = "SELECT o.id, o.total_menit, COALESCE(pg.gaji_pokok, k.salary, 0) AS gaji_pokok FROM overtime_reports o INNER JOIN karyawan k ON k.id = o.karyawan_id LEFT JOIN profil_gaji pg ON pg.karyawan_id = k.id WHERE o.id = ? AND o.status = 'disetujui'";
...
$detailLaporan = mysqli_query($conn, "SELECT o.id, o.deskripsi, GROUP_CONCAT(CONCAT(UPPER(oa.tahap), ': ', IF(oa.status = 'approved', 'disetujui', IF(oa.status = 'rejected', 'ditolak', 'menunggu')), IF(TRIM(COALESCE(oa.catatan, '')) <> '', CONCAT(' — ', oa.catatan), '')) ORDER BY oa.tahap SEPARATOR '||') AS approval_detail FROM overtime_reports o LEFT JOIN overtime_approvals oa ON oa.overtime_id = o.id WHERE $where GROUP BY o.id ORDER BY o.created_at DESC");
$gajiLembur = [];
...
document.querySelectorAll('.data-card tbody tr').forEach(row => {
    if (row.children[5]?.textContent.trim() !== 'disetujui') return;
    const action = row.children[7];
    if (!action || action.querySelector('.compensation-form')) return;
    const id = row.children[0].textContent.trim();
    const form = document.createElement('form');
    form.method = 'POST'; form.className = 'compensation-form';
    form.innerHTML = '<input type="hidden" name="csrf_token" value="<?= htmlspecialchars(tokenCsrf()); ?>"><input type="hidden" name="aksi" value="kompensasi"><input type="hidden" name="overtime_id" value="' + id + '"><input type="hidden" name="metode_perhitungan" value="per_jam"><input name="tarif_per_jam" class="overtime-rate" type="number" step="0.01"><input name="jumlah_upah" class="overtime-total" type="number" step="0.01" required><button class="btn btn-success" type="submit">Simpan Upah</button>';
    const minutes = Number(row.children[4]?.textContent.replace(/\D/g, '')) || 0;
    const hourly = Number(gajiLembur[id] || 0) / 173;
    form.querySelector('.overtime-rate').value = hourly.toFixed(2);
    form.querySelector('.overtime-total').value = (minutes / 60 * hourly).toFixed(2);
    form.querySelector('.overtime-rate').readOnly = true;
    form.querySelector('.overtime-total').readOnly = true;
    action.textContent = ''; action.appendChild(form);
});
</script>
<?php endif; ?>
