...
<section class="data-card overtime-details"><div class="data-card-header"><h2>Alasan Pengajuan</h2></div><div class="overtime-detail-list"><?php if ($detailLaporan && mysqli_num_rows($detailLaporan) > 0): ?><?php while ($detail = mysqli_fetch_assoc($detailLaporan)): ?><article class="overtime-detail-item"><strong>Lembur ID <?= (int) $detail["id"]; ?></strong><p><b>Alasan pengajuan:</b> <?= nl2br(htmlspecialchars(trim((string) ($detail["deskripsi"] ?? "")) ?: "-")); ?></p></article><?php endwhile; ?><?php else: ?><div class="notification-empty">Belum ada laporan lembur.</div><?php endif; ?></div></section>
